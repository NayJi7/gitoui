...
{/* PHOTO of the commit list home screen — graph column on the left, branches diverging, the highlighted cursor row, the bottom footer with shortcut hints, repo name on top. */}
...
{/* PHOTO gif of an interactive-rebase pass — open plan editor on a 4-commit range, grab a row with Space, reorder, change one to `reword`, type a new subject inline, Enter to apply. */}
...
{/* PHOTO of a PR detail view — Conversation tab open, author avatar inline, label chips visible, a comment card with the action ribbon highlighted, footer shortcuts on bottom. */}
