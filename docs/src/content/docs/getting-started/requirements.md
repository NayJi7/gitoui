...
gitoui targets recent Linux and macOS terminals. Windows isn't officially
...
gitoui pre-renders the commit graph as SVG/PNG and ships it inline. To see the
...
In a terminal that supports **none** of these, gitoui still works — it falls
...
- A repo with a **`github.com` remote** (any of `origin`, `upstream`, or a
  custom name — gitoui scans `git remote -v` at startup).
...
(Issues) shortcuts are silent no-ops — the rest of gitoui works fine.
