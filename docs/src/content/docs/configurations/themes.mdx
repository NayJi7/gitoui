...
{/* PHOTO of a 2×5 grid of the same commit-list view rendered with each built-in theme. Same repo, same window size — only the palette differs. */}
...
{/* PHOTO gif of live theme cycling — Configuration view open on the "theme" row, pressing ← / → cycles through built-ins then a custom theme, the whole UI repaints between each press. */}
