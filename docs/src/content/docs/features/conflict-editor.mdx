...
{/* PHOTO of the conflict editor in two-pane mode — OURS on the left (your branch's version), THEIRS on the right (incoming), conflict markers stripped, the active hunk's picker chip showing between the panes. */}
...
{/* PHOTO gif of a full conflict resolution — open editor on a multi-hunk conflict, press `o` (OURS) on one, `t` (THEIRS) on another, `b` (both) on a third, `n` to move between hunks, Enter to save. */}
