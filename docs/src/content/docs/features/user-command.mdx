...
<span class="gitoui-wordmark"><span class="git-part">git</span><span class="oui-part">oui</span></span> lets you wire up custom shell commands to numbered keybinds — useful
...
The bound key fires the command in a subshell. <span class="gitoui-wordmark"><span class="git-part">git</span><span class="oui-part">oui</span></span> suspends the TUI
...
{/* PHOTO gif of a user command firing — focus a commit, press Shift+2, the TUI suspends, the command output streams to the underlying terminal (e.g. `cargo test` running), then gitoui resumes back to the same view on exit. */}

## Limits

- Commands run with the same env as the <span class="gitoui-wordmark"><span class="git-part">git</span><span class="oui-part">oui</span></span> process — no isolation.
