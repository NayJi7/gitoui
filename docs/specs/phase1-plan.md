# gitr Phase 1 — Enhanced Viewer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Transform serie into gitr — a terminal Git viewer with mouse support, uncommitted changes visibility, inline diffs, file tree, enhanced status bar, and broader terminal compatibility.
...
- `$XDG_CONFIG_HOME/gitr/config.toml`
- `$GITR_CONFIG_FILE`

In `src/config.rs`, find the config path resolution (around line 25-50) and change:
- `SERIE_CONFIG_FILE` → `GITR_CONFIG_FILE`
- `serie/config.toml` → `gitr/config.toml`
...
```bash
git add -A && git commit -m "feat: rename project from serie to gitr"
```
...
mod tests {
    ...
    fn test_uncommitted_changes_parsing() {
        ...
        let changes = gitr::git::status::UncommittedChanges::load(dir.path()).unwrap();
    ...
    fn test_diff_parsing() {
        ...
        let entries = gitr::git::diff::DiffEntry::load_for_commit(dir.path(), &hash).unwrap();
