...
use crate::{app::AppContext, event::{AppEvent, DialogKind, GitAction, Sender, UserEvent, UserEventWithCount}, view::View};
...
impl<'a> DialogView<'a> {
    ...
    fn navigate_down(&mut self) {
        match &self.kind {
            DialogKind::Reset { .. } => {
                if self.dropdown_selected < 2 { self.dropdown_selected += 1; }
            }
            _ => {}
        }
    }
    ...
    fn navigate_up(&mut self) {
        if self.dropdown_selected > 0 { self.dropdown_selected -= 1; }
    }

    fn toggle_checkbox(&mut self) {
        match &self.kind {
            DialogKind::AddTag { .. } | DialogKind::CreateBranch { .. }
            | DialogKind::PushBranch { .. } | DialogKind::Checkout { .. }
            | DialogKind::DeleteBranch { .. } | DialogKind::PullBranch { .. }
            | DialogKind::CreateBranchFromStash { .. } | DialogKind::StashWithMessage
        ...
        let (target, action) = match &self.kind {
            DialogKind::AddTag { target } => {
                if self.input_value.trim().is_empty() {
                    self.tx.send(AppEvent::NotifyError("Tag name cannot be empty".into()));
                    return;
                }
                ...
                (target.clone(), GitAction::AddTag {
                    name: self.input_value.clone(),
                    annotated,
                    message: msg,
                })
            }
            DialogKind::CreateBranch { target } | DialogKind::CreateBranchFromStash { target, .. } => {
                if self.input_value.trim().is_empty() {
                    self.tx.send(AppEvent::NotifyError("Branch name cannot be empty".into()));
                    return;
                }
                let checkout = self.checkboxes.get(0).copied().unwrap_or(false);
                (target.clone(), GitAction::CreateBranch {
                    name: self.input_value.clone(),
                    checkout,
                })
            }
            DialogKind::Checkout { target, .. } => {
                (target.clone(), GitAction::Checkout)
            }
            DialogKind::CherryPick { target } => {
                ...
                (target.clone(), GitAction::CherryPick { no_commit, record_origin })
            }
            ...
            DialogKind::Merge { target, .. } => {
                ...
                (target.clone(), GitAction::Merge { no_ff, squash, no_commit })
            }
            DialogKind::Rebase { target } => {
                ...
                (target.clone(), GitAction::Rebase { ignore_date, interactive })
            }
            DialogKind::Reset { target } => {
                let mode = match self.dropdown_selected {
                    0 => "--soft", 1 => "--mixed", 2 => "--hard", _ => "--mixed",
                };
                (target.clone(), GitAction::Reset { mode: mode.to_string() })
            }
            DialogKind::RenameBranch { branch } => {
                ...
                (branch.clone(), GitAction::RenameBranch { new_name: self.input_value.clone() })
            }
            ...
            DialogKind::StashWithMessage => {
                let msg = if self.input_value.is_empty() { None } else { Some(self.input_value.clone()) };
                (String::new(), GitAction::Stash { message: msg })
            }
            DialogKind::CommitWithMessage => {
                if self.input_value.trim().is_empty() {
                    self.tx.send(AppEvent::NotifyError("Commit message cannot be empty".into()));
                    return;
                }
            ...
            DialogKind::ConfirmDiscardAll => {
                (String::new(), GitAction::DiscardAll)
            }
            DialogKind::ConfirmStageAll => {
                (String::new(), GitAction::StageAll)
            }
            DialogKind::ConfirmUnstageAll => {
                (String::new(), GitAction::UnstageAll)
            }
    ...
    fn build_lines(&self) -> Vec<Line<'static>> {
        ...
        match &self.kind {
            DialogKind::AddTag { target } => {
                lines.push(Line::from(vec![Span::styled("Add Tag", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Target: {}", target)));
                lines.push(Line::from(""));
                lines.push(Line::from("Tag name:"));
                lines.push(Line::from(vec![
                    Span::raw(" "),
                    Span::styled(self.input_value.clone(), Style::default().add_modifier(Modifier::UNDERLINED)),
                ]));
                lines.push(Line::from(""));
                lines.push(Line::from("Options:"));
                let annotated = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                lines.push(Line::from(format!("{} Annotated tag", annotated)));
            }
            DialogKind::CreateBranch { target } => {
                lines.push(Line::from(vec![Span::styled("Create Branch", Style::default().add_modifier(Modifier::BOLD))]));
                ...
                lines.push(Line::from(""));
                let checkout = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                lines.push(Line::from(format!("{} Checkout after creation", checkout)));
            }
            DialogKind::Checkout { target, is_branch } => {
                lines.push(Line::from(vec![Span::styled("Checkout", Style::default().add_modifier(Modifier::BOLD))]));
                let kind = if *is_branch { "branch" } else { "commit" };
                lines.push(Line::from(format!("Checkout {}: {}", kind, target)));
            }
            DialogKind::CherryPick { target } => {
                lines.push(Line::from(vec![Span::styled("Cherry Pick", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Commit: {}", target)));
                lines.push(Line::from(""));
                let record = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                let no_commit = if self.checkboxes.get(1).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
            ...
            DialogKind::Revert { target } => {
                lines.push(Line::from(vec![Span::styled("Revert Commit", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Commit: {}", target)));
            }
            DialogKind::Drop { target } => {
                lines.push(Line::from(vec![Span::styled("Drop Commit", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Commit: {}", target)));
                lines.push(Line::from(""));
                lines.push(Line::from("Warning: This will permanently remove the commit."));
            }
            DialogKind::Merge { target, is_branch } => {
                lines.push(Line::from(vec![Span::styled("Merge", Style::default().add_modifier(Modifier::BOLD))]));
                let kind = if *is_branch { "branch" } else { "commit" };
                lines.push(Line::from(format!("Merge {}: {}", kind, target)));
                lines.push(Line::from(""));
                let no_ff = if self.checkboxes.get(0).copied().unwrap_or(true) { "[x]" } else { "[ ]" };
                let squash = if self.checkboxes.get(1).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                let no_commit = if self.checkboxes.get(2).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
            ...
            DialogKind::Rebase { target } => {
                lines.push(Line::from(vec![Span::styled("Rebase", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Rebase current branch onto: {}", target)));
                lines.push(Line::from(""));
                let interactive = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                let ignore_date = if self.checkboxes.get(1).copied().unwrap_or(true) { "[x]" } else { "[ ]" };
                lines.push(Line::from(format!("{} Interactive (-i)", interactive)));
                lines.push(Line::from(format!("{} Ignore date (--ignore-date)", ignore_date)));
            }
            DialogKind::Reset { target } => {
                lines.push(Line::from(vec![Span::styled("Reset", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Reset current branch to: {}", target)));
                lines.push(Line::from(""));
                let modes = vec!["--soft", "--mixed", "--hard"];
                lines.push(Line::from("Mode:"));
                for (i, mode) in modes.iter().enumerate() {
                    let indicator = if i == self.dropdown_selected { "> " } else { "  " };
                    lines.push(Line::from(format!("{}{}", indicator, mode)));
                }
            }
            DialogKind::RenameBranch { branch } => {
                lines.push(Line::from(vec![Span::styled("Rename Branch", Style::default().add_modifier(Modifier::BOLD))]));
            ...
            DialogKind::DeleteBranch { branch, is_remote } => {
                lines.push(Line::from(vec![Span::styled("Delete Branch", Style::default().add_modifier(Modifier::BOLD))]));
                let kind = if *is_remote { "Remote branch" } else { "Branch" };
            ...
            DialogKind::DeleteTag { tag } => {
                lines.push(Line::from(vec![Span::styled("Delete Tag", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Tag: {}", tag)));
            }
            DialogKind::PushTag { tag } => {
                lines.push(Line::from(vec![Span::styled("Push Tag", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Tag: {}", tag)));
            }
            DialogKind::PullBranch { branch } => {
                lines.push(Line::from(vec![Span::styled("Pull Branch", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Branch: {}", branch)));
                lines.push(Line::from(""));
                let rebase = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                lines.push(Line::from(format!("{} Rebase (--rebase)", rebase)));
            }
            DialogKind::PushBranch { branch } => {
                lines.push(Line::from(vec![Span::styled("Push Branch", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(format!("Branch: {}", branch)));
                lines.push(Line::from(""));
                let force = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
            ...
            DialogKind::StashWithMessage => {
                lines.push(Line::from(vec![Span::styled("Stash Changes", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(""));
                ...
                lines.push(Line::from(""));
                let untracked = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                lines.push(Line::from(format!("{} Include untracked files", untracked)));
            }
            DialogKind::CommitWithMessage => {
                lines.push(Line::from(vec![Span::styled("Commit", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(""));
                ...
                lines.push(Line::from(""));
                let amend = if self.checkboxes.get(0).copied().unwrap_or(false) { "[x]" } else { "[ ]" };
                lines.push(Line::from(format!("{} Amend previous commit", amend)));
            }
            DialogKind::ConfirmDiscardFile { file } => {
                lines.push(Line::from(vec![Span::styled("Confirm Discard File", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(""));
                lines.push(Line::from(format!("Discard changes to '{}' ?", file)));
                lines.push(Line::from("This action cannot be undone."));
            }
            DialogKind::ConfirmDiscardAll => {
                lines.push(Line::from(vec![Span::styled("Confirm Discard All", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(""));
                lines.push(Line::from("Discard all unstaged changes?"));
                lines.push(Line::from("This action cannot be undone."));
            }
            DialogKind::ConfirmStageAll => {
                lines.push(Line::from(vec![Span::styled("Confirm Stage All", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(""));
                lines.push(Line::from("Stage all changes?"));
            }
            DialogKind::ConfirmUnstageAll => {
                lines.push(Line::from(vec![Span::styled("Confirm Unstage All", Style::default().add_modifier(Modifier::BOLD))]));
                lines.push(Line::from(""));
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(""));
        lines.push(Line::from(vec![Span::styled("Enter:confirm Esc:cancel", Style::default().fg(Color::Gray))]));
