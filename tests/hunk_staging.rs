...
fn load_staged_tags_hunks_as_staged() {
    ...
    // Stage a change.
    let mut lines: Vec<String> = baseline_file_30_lines().lines().map(|s| s.to_string() + "\n").collect();
    ...
    for h in &entry.hunks {
        assert_eq!(h.origin, HunkOrigin::Staged, "every loaded hunk must be tagged Staged");
    }
}
...
fn load_for_commit_keeps_origin_other() {
    ...
    for e in &entries {
        for h in &e.hunks {
            assert_eq!(h.origin, HunkOrigin::Other, "committed diff hunks must default to Other");
        }
    }
}
...
fn combined_loader_returns_none_for_unchanged_file() {
    ...
    assert!(combined.is_none(), "unchanged file should yield no combined diff");
}
...
fn combined_loader_merges_staged_and_unstaged_sorted() {
    ...
    let combined = DiffEntry::load_combined_uncommitted_for_file(dir.path(), "a.txt", 3)
        .unwrap()
        .expect("combined diff expected");
    assert!(combined.hunks.len() >= 2, "expected at least two hunks, got {}", combined.hunks.len());
    ...
    assert!(origins.contains(&HunkOrigin::Staged), "missing staged hunk: {origins:?}");
    assert!(origins.contains(&HunkOrigin::Unstaged), "missing unstaged hunk: {origins:?}");
}
...
fn stage_hunk_moves_change_to_index() {
    ...
    assert!(cached.contains("CHANGE_TOP"), "staged diff missing CHANGE_TOP: {cached}");
    assert!(!cached.contains("CHANGE_BOTTOM"), "staged diff should not include CHANGE_BOTTOM yet");
...
fn unstage_hunk_moves_change_out_of_index() {
    ...
    let cached = git_stdout(dir.path(), &["diff", "--cached", "a.txt"]);
    assert!(!cached.contains("STAGED_A"), "first hunk should have been unstaged");
    assert!(cached.contains("STAGED_B"), "second hunk should remain staged");

    let unstaged = git_stdout(dir.path(), &["diff", "a.txt"]);
    assert!(unstaged.contains("STAGED_A"), "first hunk should be in working tree diff");
}
...
fn stage_then_unstage_round_trip_restores_state() {
    ...
    let after_stage_combined = DiffEntry::load_combined_uncommitted_for_file(dir.path(), "a.txt", 3)
        .unwrap()
        .unwrap();
    ...
    assert_eq!(final_unstaged, initial_unstaged, "unstaged diff should be restored");
    assert_eq!(final_staged, initial_staged, "staged diff should be restored");
}
...
fn stage_hunk_preserves_addition_deletion_counts() {
    ...
    assert_eq!(adds, stg_adds, "staged addition count must equal hunk addition count");
    assert_eq!(dels, stg_dels, "staged deletion count must equal hunk deletion count");
}
...
fn stage_hunk_twice_second_call_is_a_noop_error() {
    ...
    assert!(second.is_err(), "second stage of same hunk should fail cleanly");
}
...
fn stage_hunk_does_not_disturb_other_files() {
    ...
    assert!(!cached.contains("b.txt"), "b.txt must remain unstaged after staging a.txt only");
}
